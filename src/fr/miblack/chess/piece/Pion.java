...
public class Pion extends Piece
{
	...
	{
		LinkedList<Position> myListOfPosAtteignable = this
				.positionAccessibleChessboard( maPartie.getMyChessboard() );
		LinkedList<Coup> listOfCoup = super.getCoupPossible( PieceDepart,
				maPartie );
	...
	{
		LinkedList<Position> myList = positionAccessibleCouleur( color
				.getColor() );
		return myList;
	}
		...
		if ( this.getPlayed() == 0 )
		{
			if ( estValide( getY() + (1 * mult) ) )
			{
				if ( chess.getPiecePosition( getX(), (getY() + ((1) * mult)) ) != null )
				{
					...
					{
						myList.remove( Position.getPosition( getX(), getY()
								+ (2 * mult) ) );
					}
				}
			}
	...
	public LinkedList<Position> whatCanIEat( Echiquier chess )
	{
		...
		if ( (this.getX() + 1) <= 7 )
		{
			...
			{
				if ( chess.getPiecePosition( this.getX() + 1, this.getY()
						+ (1 * mult) ) != null )
				{
					if ( !verifColor( chess, this.getX() + 1, this.getY()
							+ (1 * mult) ) )
					{
						if ( !(chess.getPiecePosition( this.getX() + 1,
								this.getY() + (1 * mult) ) instanceof Roi) )
							myList.add( Position.getPosition( this.getX() + 1,
									this.getY() + (1 * mult) ) );
					}
				}
			}
